...
export default function AudioTabs({ activeTab, setActiveTab }) {
  ...
  const tabs = [
    { label: "Transcription", icon: TranscriptionIcon },
    { label: "Waveform", icon: WaveformIcon },
    { label: "Comments", icon: CommentsIcon },
  ];
  ...
  return (
    <div className="audio-tabs">
      {tabs.map((tab) => (
        <button
          key={tab.label}
          className={`audio-tab ${activeTab === tab.label ? "active" : ""}`}
