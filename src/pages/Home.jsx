...
import RecordMenu from '../components/Record/RecordMenu';
...
export default function Home() {
  ...
  useEffect(() => {
    const containers = Array.from(
      document.querySelectorAll(".audios-scroll-container")
    );
    ...
    const update = (el) => {
      ...
      if (!atStart && !atEnd) {
        el.classList.add("can-scroll-both");
      } else if (!atStart) {
    ...
    return () => {
      containers.forEach((el) => {
        el.removeEventListener("scroll", handlers.get(el));
      });
      observers.forEach((o) => o.disconnect());
    };
  }, [audios, favouriteAudios]);
                  ...
                  <div className="audios-container">
                    {audios.length === 0 ? (
                      <EmptyState
                        ...
                        secondaryText="Record Audio"
                        secondaryOnClick={() => console.log("Hewlloo")}
                      />
                    ) : (
